...
BEGIN
    RETURN QUERY EXECUTE format(
        'SELECT 
            date_trunc(%L, cs.begin) AS period,
			cs.nm_id as nm_id,
			c.title as title,
			c.vendor_code as vendor_code,
			sum(cs.open_card_count) as open_card_count,
			sum(cs.add_to_cart_count) as add_to_cart_count,
			sum(cs.orders_count) as orders_count,
			sum(cs.orders_sum_rub) as orders_sum_rub,
			sum(cs.buyouts_count) as buyouts_count,
			sum(cs.buyouts_sum_rub) as buyouts_sum_rub,
			sum(cs.cancel_count) as cancel_count,
			sum(cs.cancel_sum_rub) as cancel_sum_rub
        FROM cards_stat cs
		JOIN cards c on c.nm_id = cs.nm_id
        GROUP BY period, cs.nm_id, c.title, c.vendor_code
        ORDER BY period, cs.nm_id', 
        period_type);
END;
$$;
